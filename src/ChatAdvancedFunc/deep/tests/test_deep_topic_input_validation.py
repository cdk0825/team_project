...
def test_deep_topic_input_validation(driver, topic_input, expect_error):
    ...
    print("\n==============================")
    print("[TEST START] DEEP Topic Input Validation")
    ...
    print(f"[STEP] 주제 입력값 길이: {len(topic_input)}")
    ...
    if expect_error:
        ...
        print(f"[RESULT] 주제 에러 메시지: {error_text}")
        assert error_text == "1자 이상 500자 이하로 입력해주세요.", \
            "주제 길이 초과 시 에러 메시지가 표시되지 않음"

    print("[TEST END] DEEP Topic Input Validation")
    print("==============================\n")
