...
def test_input_reset_rules(driver):
  ...
  try:  
    ...
    if not download_btn.is_displayed():
        logger.error("[ERROR] 생성 완료 후 다운로드 버튼이 노출되지 않음")
    ...
    if topic_value != "":
        logger.error(f"[ERROR] 주제 입력값 초기화 실패 - actual: '{topic_value}'")
    if instruction_value != "":
        logger.error(f"[ERROR] 지시사항 입력값 초기화 실패 - actual: '{instruction_value}'")
    if section_value != "":
        logger.error(f"[ERROR] 섹션 입력값 초기화 실패 - actual: '{section_value}'")
    if slide_value != "":
        logger.error(f"[ERROR] 슬라이드 입력값 초기화 실패 - actual: '{slide_value}'")
