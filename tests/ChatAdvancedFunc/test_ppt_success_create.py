...
def test_success_create(driver, download_dir):
    ...
    if is_enabled is not True:
        logger.error("[ERROR] 필수 입력값 입력 후 생성 버튼이 비활성화됨")

    assert is_enabled is True, "생성 버튼이 활성화되지 않음"
    logger.info("[ASSERT PASS] 생성 버튼 활성화 확인")
    ...
    if not download_btn.is_displayed():
        logger.error("[ERROR] PPT 생성 완료 후 다운로드 버튼이 노출되지 않음")

    assert download_btn.is_displayed(), "다운로드 버튼 미노출"
    ...
    if downloaded_file is None:
        logger.error("[ERROR] 다운로드 디렉토리에 파일이 생성되지 않음")

    assert downloaded_file is not None, "파일이 다운로드되지 않음"

    if not downloaded_file.endswith(".pptx"):
        logger.error(f"[ERROR] 다운로드 파일 확장자 오류: {downloaded_file}")

    assert downloaded_file.endswith(".pptx"), (
        f"다운로드 파일 확장자 오류: {downloaded_file}"
    )
    
    logger.info("[ASSERT PASS] PPT 파일 정상 다운로드 확인")
